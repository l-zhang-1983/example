...
public class RegcodeRealm extends AuthorizingRealm {
	Logger logger = LoggerFactory.getLogger(RegcodeRealm.class);
//	@Autowired
...
//		authorizationInfo.addRoles(permissionService.getUserRolesByIdentityNo(username));
...
//		CpsUser user = userService.getUserByIdentifyNo(token.getUsername());
		
//		if (null != user && user.getUserStatus() == CpsConstants.STATUS_REGCODE) {
...
//			AuthenticationInfo authcInfo = new SimpleAuthenticationInfo(spc, MD5Util.md5Hash(user.getRegCode()));
